...
class TestWS(unittest.TestCase):
    ...
    @patch('ws_sdk.web.requests')                            # Injecting mock for requests module. Object name is mock_request
    def test___call_api__(self, mock_requests):
        with patch('ws_sdk.web.requests.post') as patched_post:  # Mocking post method so return value will include status_code attribute
    ...
    def test_get_all_products_with_product_token(self, mock_call_api):
        with patch('ws_sdk.web.WS', new_callable=PropertyMock) as ws_class:
